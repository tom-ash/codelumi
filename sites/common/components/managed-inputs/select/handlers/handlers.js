export function onKeyDownHandler(e) {
  ...
  if (keyCode === 38 || keyCode === 40) {
    ...
    this.options.current.scrollTop = computeScroll.call(
      this,
      preSelected,
      keyCode,
    );
...
function computeScroll(preSelectedIndex, keyCode) {
  ...
  switch (preSelectedIndex) {
    ...
    default:
      ...
      if (
        keyCode === 40 &&
        preSelectedOffset + preSelectedHeight > optionsScroll + optionsHeight
      ) {
        const diff =
          ((preSelectedOffset + preSelectedHeight - optionsHeight) %
            preSelectedHeight) -
          preSelectedHeight;
