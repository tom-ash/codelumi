...
import { controlProvider } from './components/control-provider/control-provider';
...
import { PromoteButton } from '../../../new/form/view/components/success/components/promote';
import { ShareOnFacebookButton } from '../../../../../../../common/components/support/facebook-sharer-button/facebook-sharer-button';
...
class AnnouncementIndexMy extends React.Component {
  ...
  render() {
    // @ts-ignore
    const { amount, offset, lang, isMobile, shareOnFacebookButtonLabel } =
      this.props;

    const toValue = offset + 10 < amount ? offset + 10 : amount;
    let whichAnnouncements = `${offset + 1} - ${toValue} ${{ pl: 'z', en: 'of' }['pl']}`;
    if (offset + 1 === amount)
      whichAnnouncements = `${offset + 1} ${{ pl: 'z', en: 'of' }['pl']}`;
          ...
          {this.props.announcements.map((announcement, index) => {
            ...
            return (
              <div className="listing-index-tile-container">
                ...
                <ShareOnFacebookButton {...facebookSharerButtonProps} />
                <FloatClear />
                {/* @ts-ignore */}
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withStyles(styles)(AnnouncementIndexMy));
