...
export const SocialImage = () => {
  ...
  // @ts-ignore
  const b2bCurrency = b2bCurrencySelectOptions.find(
    (currency) => currency.value === b2bCurrencyValue,
  ).text;
  ...
  const industryObject = useMemo(() => {
    return industries.find(
      (ind: { value: string; label: string; icon: string }) => {
  ...
  const cooperationModeObject = useMemo(() => {
    return cooperationModes.find(
      (coopMode: { value: string; icon: string }) => {
  ...
  const logoSrc =
    typeof logo === 'object' ? logo.objectUrl : `${logo}?salt=${Math.random()}`;
