...
function Skills() {
  ...
  const searchedSkills = useMemo(() => {
    return selectableSkills.filter((selectableSkill: ExtendedSkillProps) => {
      ...
      return (
        !selectableSkill.isSelected &&
        selectableSkill.display.toLowerCase().match(regexp)
      );
    });
  ...
  const onSelectableSkillLevelClicked = useCallback(
    (params: Pick<SkillProps, 'value' | 'level'>) => {
      ...
      const selectedableSkillIndex = selectableSkills.findIndex(
        (selectableSkill: ExtendedSkillProps) =>
          selectableSkill.value === value,
      );
  ...
  return (
    <section id="skills" className={classNames.join(' ')}>
      <SectionHeading
        name="skills"
            ...
            <div className="skill-list">
              {searchedSkills.map((skill: SkillProps) => (
                <Skill
                  ...
                  onLevelClicked={(params) =>
                    onSelectableSkillLevelClicked(params)
                  }
                />
              ))}
            </div>
          ...
          <div id="selected-skills">
            {selectedSkills.map((skill: SkillProps, index: number) => (
              ...
              >
                ...
                <div className="up" onClick={() => moveUp(index)}>
                  <SVG name="arrowRight" />
                </div>
                <div className="down" onClick={() => moveDown(index)}>
                  <SVG name="arrowRight" />
                </div>
              </Skill>
            ))}
