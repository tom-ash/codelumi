...
const removedPinHovering = () => {
  const unhoveredPinsHTMLCollection = document.getElementsByClassName('pin hovered')
...
export const useMouseHovering = () => {
  useEffect(() => {
    ...
    function addHoverListeners(scrollContainer, hover) {
      ...
      document.addEventListener('mousemove', onMouseMoveHover, { passive: true })
      ...
      scrollContainer.addEventListener('scroll', onScrollHover, { passive: true })
    }

    return () => {
      // @ts-ignore
      document.removeEventListener('mousedown', onMouseMoveHover, { passive: true })
