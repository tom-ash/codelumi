...
const ListingTileShowSecondary: ListingTileShowSecondaryInterface = props => {
  const { rooms, category, floor, totalFloors, availabilityDate } = props
    ...
    <div className='secondary'>
      ...
      {floor !== null && (
        <div className='item floor'>
          <SVG name='layers' /> {presentFloor({ floor })['pl']}
        </div>
      )}
      {totalFloors !== null && (
        // TODO: LANG!
        <div className='item total-floors'>
          <SVG name='apartment' /> {presentTotalFloors({ totalFloors })['pl']}
        </div>
      )}
